...
Order Factors in logical order for display in plots.
...
loans %>%
  select(LoanOriginalAmount, Term, BorrowerAPR, EstimatedReturn,
         ...
         Investors, PercentFunded, InvestmentFromFriendsCount,
...
ggplot(loans, aes(x=IncomeRange, y=LoanOriginalAmount)) +
  ...
  labs(title='Histogram of Loan Amount by Income Range',
       x='Income Range',
       y='Loan Amount')
...
loans %>%
  filter(ListingCreationDate.dt >= as.POSIXct('2009-07-01',
                                              tz='UTC',
                                              format='%Y-%m-%d')) %>%
...
ggplot(., aes(x=IncomeRange, y=ListingCategory, fill=LoanAmountCatPct)) +
  ...
  labs(x='Income Range', y='Listing Category',
       ...
       title='Proportion of Loan Dollars for Listings by Income')
```
...
### Estimated Return of Mean Loan by Prosper Rating 
```{r}
loans %>%
  filter(ListingCreationDate.dt >= as.POSIXct('2009-07-01',
                                              tz='UTC',
  ...
  summarize(MeanReturnDollars=mean(EstimatedReturnDollars),
            MedianReturnDollars=median(EstimatedReturnDollars))
...
ggplot(., aes(x=LoanOriginalAmount, y=Investors)) +
  geom_point(aes(color=ProsperRatingAlpha), alpha=0.8) +
