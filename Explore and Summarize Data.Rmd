...
ggplot(., aes(x=IncomeRange)) +
  ...
  labs(title='Count of Prosper Ratings by Income Range',
       x='Income Range',
       y='Count',
       fill='Prosper Rating')
...
### Default Rates by Prosper Rating
```{r Default Rates by Rating, echo=FALSE}
loans %>%
  mutate(month=format(ListingCreationDate.dt, '%m'),
         year=format(ListingCreationDate.dt, '%Y'),
         yearmonth=as.integer(format(ListingCreationDate.dt, '%Y%m'))) %>%
  mutate(post200907 = ifelse(yearmonth>=200907, TRUE, FALSE)) %>%
  group_by(post200907) %>%
  summarize(n=n())
...
loans %>%
  mutate(month=format(ListingCreationDate.dt, '%m'),
         ...
         yearmonth=as.integer(format(ListingCreationDate.dt, '%Y%m'))) %>%
  mutate(post200907 = ifelse(yearmonth>=200907, TRUE, FALSE)) %>%
  group_by(yearmonth, ProsperScore) %>%
  summarize(n=n())  
